...
BOT_TOKEN = environ.get('BOT_TOKEN', '6559513905:AAGpz_JC8WUZlQCbbGilS1qCQwnBbF6u8hQ')
...
ADMINS = [5894098166]
CHANNELS = [-1001998108714]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', "").split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL', '-1001967738594')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else auth_channel
AUTH_GROUPS = [int(admin) for admin in environ.get("AUTH_GROUPS", "").split()]

# MongoDB information
DATABASE_URI = 'mongodb+srv://zoro:lost@cluster0.4zwdyde.mongodb.net/?retryWrites=true&w=majority'
DATABASE_NAME = environ.get('DATABASE_NAME', 'terabhai')
